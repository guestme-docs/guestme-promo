...
export default function TipsPromotionDetailClient({ promotionId }: { promotionId: string }) {
  ...
  // ID текущего официанта (в реальном приложении это будет из контекста/состояния)
  ...
  // Получаем продажи по акции только для текущего официанта
  const promotionSales = useMemo(() => {
    return liveSales.filter(sale => 
      sale.promotionId === promotion.id && sale.waiterId === currentWaiterId
    );
  }, [promotion.id, currentWaiterId]);

  // Вычисляем статистику только для текущего официанта
  const waiterSalesCount = useMemo(() => {
    return promotionSales.reduce((sum, sale) => 
      sum + sale.items.reduce((itemSum, item) => itemSum + item.quantity, 0), 0
    );
  }, [promotionSales]);

  // Вычисляем мотивацию только для текущего официанта (60% от общей мотивации)
  const waiterMotivation = useMemo(() => {
    return promotionSales.reduce((sum, sale) => 
      sum + sale.items.reduce((itemSum, item) => {
        const totalMotivation = (item.motivation?.amount || 0) * item.quantity;
        const waiterShare = Math.round(totalMotivation * 0.6); // 60% для официанта
        return itemSum + waiterShare;
      }, 0), 0
    );
  }, [promotionSales]);
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 4, alignItems: 'stretch' }}>
        ...
        <Box sx={{ height: '100%' }}>
          <TipsPromotionCard
            ...
            salesCount={waiterSalesCount}
            motivationToPay={{ amount: waiterMotivation, currency: 'RUB' }}
            onClick={() => {}}
            ...
            {paginatedSales.map((sale) => {
              ...
              return (
                <SaleEventCard
                  ...
                  onRestaurantClick={() => {
                    router.push(`/tips/restaurants/${sale.restaurantId}`);
                  }}
                />
              );
            })}
          ...
          <Box>
            ...
            <Box sx={{ space: 2 }}>
              {topRestaurants.map(({ restaurant, sales, motivation }, index) => (
                <TopItemCard key={restaurant.id} sx={{ mb: 2 }} onClick={() => router.push(`/tips/restaurants/${restaurant.id}`)}>
                  <CardContent sx={{ p: 2 }}>
