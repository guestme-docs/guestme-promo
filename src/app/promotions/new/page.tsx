...
export default function CreatePromotionPage() {
  ...
  const addProducts = (productIds: string[]) => {
    const newProducts = productIds
      ...
      .map(productId => ({
        productId,
        motivation: 10000, // 100 рублей в копейках
        waiterShare: 60,
        adminShare: 10,
      }));
                        ...
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 2, mt: 2 }}>
                            <StyledTextField
                              label="Мотивация (руб.)"
                              type="number"
                              value={productMotivation.motivation / 100 || ''}
                              onChange={(e) => {
                                ...
                                if (value === '') {
                                  ...
                                  if (!isNaN(numValue) && numValue >= 0) {
                                    updateProductMotivation(productMotivation.productId, 'motivation', Math.round(numValue * 100));
                                  }
                                }
                              }}
                              size="small"
                          ...
                          <Box sx={{ mt: 2, p: 2, backgroundColor: 'var(--motivation-background)', borderRadius: 4 }}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                              <span>Официанту: <strong>{formatMoneyRUB({ amount: Math.round(productMotivation.motivation * productMotivation.waiterShare / 100), currency: 'RUB' })}</strong></span>
                              <span>Администратору: <strong>{formatMoneyRUB({ amount: Math.round(productMotivation.motivation * productMotivation.adminShare / 100), currency: 'RUB' })}</strong></span>
                              <span>GuestMe: <strong>{formatMoneyRUB({ amount: Math.round(productMotivation.motivation * 0.3), currency: 'RUB' })}</strong></span>
                            </Box>
